...
<head>
    ...
    <title pageTitleCaseList>Staff - Case List</title>
...
<body class="text-left" >
 <%@ include file="./common/CacheControl.jsp"%>
                        ...
                        <div id="bodyMainCard" class="card mb-4">
                           ...
                           <div class="row pt-4 pr-3  mt-2 ml-2">
									...
									<div class="col-md-12  pt-0 pr-4 ">
									  <p id="pageSpanHeadingText1" class="pt-0 pb-0 mt-0 mb-0">We will confirm the matter information and contact / report the business.</p>
                                      <p id="pageSpanHeadingText2" class="pt-0 pb-0 mt-0 mb-0"><span id="pageSpanHeadingSubTextPrevious" class="case-info-notice">Previous</span><span id="pageSpanHeadingSubTextConfirmedandReported"> : Confirmed and reported the day before　</span><span id="pageSpanHeadingSubTextOut" class="case-info-notice">Out</span><span id="pageSpanHeadingSubTextDepartureReported"> : Departure reported</span>　<span id="pageSpanHeadingSubTextArrival" class="case-info-notice">Arrival</span> <span id="pageSpanHeadingSubTextArrivalReported">: Arrival reported　</span><span id="pageSpanHeadingSubTextEnd" class="case-info-notice">End</span> <span id="pageSpanHeadingSubText">: Reported end of work</span></p>
							       </div>
							  </div>
    ...
    <script>
      ...
      // const implementId = localStorage.getItem('implementId') ;
      ...
      const today =  currentYear+"-"+currentMonthText+"-"+currentDate;
      const monthStartDate =  currentYear+"-"+currentMonthText+"-" +"01";
      const monthEndDate =  currentYear+"-"+currentMonthText+"-"+ daysInMonth ;
    	...
    	if(result.data.length != 0)
    		{
    		  $("#CaseListTable tbody").empty();
    		  	 ...
    		  	 for(x = 0;x<result.data.length;x++)
    		     {
    		  	   ...
    		  	   let assignmentId = result.data[x].assignmentId;
    		  	   ...
    		  	   let rowVal =  x+1 ;
    		       let newRow  = '<tr onclick="viewCase('+staffID+','+implementId+',\''+implementDate+'\',\''+assignmentId+'\')" id="caseListTableTr" name="caseListTableTr[]" class="clickable-row" data-href="case-view.jsp">'+
		    		 		        '<td id="shiftScheduleDetailDate"  name="shiftScheduleDetailDate[]" style="width: 10%;text-align:center" class="p-1" >'+ rowVal +'</td>'+
			    		              ...
			    		              '</td>'+
			    		              '<td style="width: 45%;">'+
	    		                        '<div class="row pl-3"><span name="spanWorkLoaction[]">Work Loaction : </span>'+workLocation+'</div>'+
	    		                        '<div class="row pl-3"><span name="spanWorkAddress[]">Work Address : </span>'+workAddress+'</div>'+
	    		                        '<div class="row pl-3">'+scheduledStartTime+' ~ '+scheduledEndTime+'</div>'+
	    		                        '<div class="row pl-3"><span name="spanDepartureTime[]">Departure Time : </span>'+scheduledDepartureTime+'</div>'+
			    		              '</td>'+
		    		 		'</tr>';
    		 		
    		 		$("#CaseListTable tbody").append(newRow);
    		 	 } 
    		     loadSelectedLanguage(getSiteLanguage(),"item-list");
    	 }else
          {
    		 // onloadShiftSchedule(year,nextMonthYear,thisMonthName,thisMonthNumber,nextMonthName,nextMonthNumber);  
          } 
    ...
    function viewCase(staffID,implementId,implementDate,assignmentId)
    {
