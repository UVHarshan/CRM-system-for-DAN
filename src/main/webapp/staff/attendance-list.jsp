...
<head>
    ...
    <title id="pageTitleAttendanceList">Staff - Attendance List</title>
...
<body class="text-left" >
 <%@ include file="./common/CacheControl.jsp"%>
                        ...
                        <div id="bodyMainCard" class="card mb-4">
                           ...
                           <div class="row pt-4 pr-3  mt-2 ml-2">
									<div class="col-sm-12 TopActionButDiv mr-4  "></div>
                         	          <div class="col-md-4 " >
                                        <h1 > <i class="fa fa-file-alt  text-30" role="generic"></i><span id="pageSpanHeadingCaseList"> Attendance List</span> </h1>
									...
									<div class="col-md-12  pt-0 pr-4 ">
									  <p id="pageSpanHeadingText1" class="pt-0 pb-0 mt-0 mb-0">You can check the monthly attendance list.</p>
                                      <p id="pageSpanHeadingText2" class="pt-0 pb-0 mt-0 mb-0">Tap a row to display the monthly attendance details screen.</p>
							       </div>
							  </div>
                             <div class="row  ml-2 mr-2">
                             <div class="col-md-12  pt-0 pr-4">
                             <div class="table-responsive pt-2">
								<table class="display table table-striped table-bordered hover" id="AttendanceListTable" style="width: 100%">
									<thead id="tbodyIdAttendanceListTableHead">
										<tr >
										<th class="text-center" style="width: 20%;" id="tableTHNo" >No</th>
										<th class="TableTHMinWidth text-center" style="width: 80%;" id="tableTHMonth">Month</th>
										</tr>
									</thead>
								    <tbody id="tbodyIdAttendanceListTableBody">
								     <tr id="AttendanceListTableTr" >
										  <td id="shiftScheduleDetailDate"  name="shiftScheduleDetailDate[]" style="width: 20%;" class="p-1" >
										  &nbsp;
										  </td>
										  <td style="width: 80%;">
										  &nbsp; 
    ...
    <script>
	  ...
	  dataNodeLoad["implementEndDate"] = monthEndDate ; 
  	  ...
  	  loadTable();
           ...
           function loadTable()
           {
        		 $("#AttendanceListTable tbody").empty();
 		  		 console.log("loadTable() Runs");
     		  	 for(x = 0;x<6;x++)
     		     {
     		  	   let AttendanceYear  = currentYear; 
     		  	   let AttendanceMonth = currentMonth - x;
     		  	   if(AttendanceMonth<1)
     		  	   {
     		  		AttendanceYear = currentYear-1;
     		  		AttendanceMonth = ((AttendanceMonth + 12)%13)
     		  	   }
   		  	       
     		  	   let rowVal =  x+1 ;
     		       let newRow  = '<tr onclick="viewAttendance('+staffID+','+AttendanceYear+',\''+AttendanceMonth+'\')" id="AttendanceListTableTr" name="AttendanceListTableTr[]" class="text-center">'+
					             '<td id="shiftScheduleDetailDate"  name="shiftScheduleDetailDate[]" style="width: 20%;" class="p-1" >'+rowVal+'</td>'+
					              '<td style="width: 80%;">'+AttendanceYear+" - "+monthArrayW[AttendanceMonth]+'</td>'+
				                 '</tr>';
     		 		
     		 		$("#AttendanceListTable tbody").append(newRow);
     		 	 }   
        	   
           }   
           
           
           function viewAttendance(staffID,AttendanceYear,AttendanceMonth)
           {
       		let url = "attendance-monthly-details.jsp";
       		let form = $('<form action="' + url + '" method="post">' +
       		  '<input type="hidden" name="staffId" value="'+staffID+'" />' +
       		  '<input type="hidden" name="AttendanceYear" value="'+AttendanceYear+'" />' +
       		  '<input type="hidden" name="AttendanceMonth" value="'+AttendanceMonth+'" />' +
       		  '</form>');
       		$('body').append(form);
       		form.submit();
           } 
    	...
    	if(result.data.length != 0)
    		{
    		  $("#CaseListTable tbody").empty();
    		     ...
    		     {
    		  	   let implementId = result.data[x].implementId;
    		  	   ...
    		  	   let rowVal =  x+1 ;
    		       let newRow  = '<tr onclick="viewCase('+staffID+','+implementId+',\''+implementDate+'\')" id="caseListTableTr" name="caseListTableTr[]" class="clickable-row" data-href="case-view.jsp">'+
    ...
    function viewCase(staffID,implementId,implementDate)
    {
		...
		let form = $('<form action="' + url + '" method="post">' +
		  ...
		  '<input type="hidden" name="implementDate" value="'+implementDate+'" />' +
		  '</form>');
		$('body').append(form);
		form.submit();
    }
