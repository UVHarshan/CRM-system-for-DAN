...
<title id="pageTitleQuotationRegistration">Quotation
	Registration</title>
<link
...
<body class="text-left">
									...
									<div class="col-md-6">
										<h1>
											<i class="fa fa-file-alt text-30" role="generic"></i><span
												id="pageSpanHeadingQuotationRegistration"> Quotation
												Registration </span>
										</h1>

										<p class="pb-0 mb-0"
											id="pageSpanSubHeadingQuotationRegistration">You can
											revise a quote, create another quote or a case related to
											this quote.</p>
									</div>
									<div class="col-md-6">
															...
															<div class="col-sm-12 col-md-6 col-lg-6 pt-2 pl-3 pr-3">
																...
																<div>
																	<select id="quotationConsumptionTax"
																		...
																		role="combobox">
																		<option data-rate="0" value=""
																			id="selectOptionSelectConsumptionTax">Select
																			...
																			<tr id="quotationDetailsTaxableTrId">
																				...
																				<td class="center-content"><label
																					class="container"> <input type="checkbox"
																						...
																						class="checkmark" style="margin: auto;"></span>
																				</label></td>
																				<td><input
	...
	<script>
						...
						function() {
							...
							$
									.ajax({
										...
										url : serverHTTP
												+ 'MasterAdminTax/GetConsumptionTax',
										method : 'GET',
										...
										success : function(result, status,
											...
											if (result.status == 200) {
												...
												var SalesRepresentative = document
														.getElementById('quotationSalesRepresentative');

												if (result.data != null) {

													for (i = 0; i < result.data.length; i++) {
														SalesRepresentative.options[SalesRepresentative.options.length] = new Option(
																result.data[i].name,
																result.data[i].value);
													}
							...
							$
									.ajax({
										...
										url : serverHTTP
												+ 'Quotation/SearchQuotationTempInfo',
										method : 'POST',
										contentType : 'application/json',
										data : JSON.stringify({
											"quotationTemplate" : {
										...
										success : function(result, status,
											...
											if (result.status == 200) {
												...
												console
														.log("Sorry SearchQuotationTempInfo error");
											}
										},
										error : function(error) {
											console.log(JSON.stringify(error));
									...
									.change(
											function() {
												...
												templateID = $(
														"#quotationTemplateID")
														.val();
												let dataSet = '{"quotationTemplate": {"id": '
														+ templateID + '}}';
												$
														.ajax({
															beforeSend : function(
															...
															url : serverHTTP
																	+ 'Quotation/GetQuotationTempInfo',
															method : 'POST',
															contentType : 'application/json',
															data : dataSet,
															dataType : 'json',
							...
							$("#quotationDispatchClassification").change(
									function() {
										loadDispatch();
									});
		...
		function getQuotationInfo(result) {
			...
			document
					.getElementById("tableQuotationDetailsSubtotalNonTaxableVal").innerHTML = result.data.subTotalNonTaxable;
		...
		function getQuotationItem(result) {
			...
			for (i = 0; i < taxableItemsCount; i++) {
				...
				let newRow = '<tr id="quotationDetailsTaxableTrId">'
						...
						+ '<td class="" name="itemTaxableCounter[]">'
						+ (i + 1)
						+ '</td>'
				...
				majorItemsTaxableNodes = document
						.getElementsByName("majorItemsTaxable[]");
				majorItemsTaxableNodes[i].checked = result.data.quotationItemsTaxable[i].majorItem;
			}
			...
			for (i = 0; i < nonTaxableItemsCount; i++) {
				...
				let newRow = '<tr id="quotationDetailsNonTaxableTrId">'
						...
						+ '<td class="" name="itemNonTaxableCounter[]">'
						+ (i + 1)
						+ '</td>'
						+ '<td  class="center-content">'
						+ '<label class="container">'
				...
				majorItemsNonTaxableNodes = document
						.getElementsByName("majorItemsNonTaxable[]");
				majorItemsNonTaxableNodes[i].checked = result.data.quotationItemsNonTaxable[i].majorItem;
			}
		...
		function getQuotationTempInfo(result) {
			...
			for (i = 0; i < taxableItemsCount; i++) {
				...
				let newRow = '<tr id="quotationDetailsTaxableTrId">'
						+ '<input type="hidden" name="quotationItemTaxableID[]" value="">'
						+ '<td class="" name="itemTaxableCounter[]">'
						+ (i + 1)
						+ '</td>'
						+ '<td  class="center-content">'
			...
			for (i = 0; i < nonTaxableItemsCount; i++) {
				...
				let newRow = '<tr id="quotationDetailsNonTaxableTrId">'
						+ '<input type="hidden" name="quotationItemNonTaxableID[]" value="">'
						+ '<td class="" name="itemNonTaxableCounter[]">'
						+ (i + 1)
						+ '</td>'
						+ '<td  class="center-content">'
			...
			for (i = 0; i < ItemTaxableAmount.length; i++) {
				if (ItemTaxableAmount[i].innerHTML.trim() != "") {
					TaxableSubtotal = TaxableSubtotal
							+ parseFloat(ItemTaxableAmount[i].innerHTML.trim());
		...
		function addTaxableQuotationItemRow() {
			...
			$clone.find('td[name="quotationItemTaxableAmountMoney[]"]')
					.html('');
			$clone.find('input[name="majorItemsTaxable[]"]').prop('checked',
					false);
		...
		function addNonTaxableQuotationItemRow() {
			...
			$clone.find('td[name="quotationItemNonTaxableAmountMoney[]"]')
					.html('');
			$clone.find('input[name="majorItemsNonTaxable[]"]').prop('checked',
					false);
			$clone.find('td:eq(3)').removeClass("errorVal");
			$clone.find('td:eq(4)').removeClass("errorVal");
			...
			if (quotationId != 0) {
				...
				if (quotationDispatchClassification != 4) {
					var workingHoursStart = document
							.getElementById("workingHoursStart").value.trim();
					...
					var workingHoursEnd = document
							.getElementById("workingHoursEnd1").value.trim();
					var businessContent = document
				...
				var requiredNumberOfPeople = document
						.getElementById("requiredNumberOfPeople").value.trim();
						...
						.ajax({
							...
							success : function(result, status, request) {
								if (result.status == 202) {
									...
									checkMessage('success', 'N0003', 'N0000',
											quotationNumber, "");
		...
		function removeTaxableRow(element) {
			if ($('#QuotationDetails_table tr').length > 7) {
				...
				let counterNode = document
						.getElementsByName("itemTaxableCounter[]");
		...
		function removeNonTaxableRow(element) {
			if ($('#QuotationDetailsNonTaxable_table tr').length > 8) {
				$(element).closest('tr').remove();
				let counterNode = document
						.getElementsByName("itemNonTaxableCounter[]");
				for (var i = 0; i < counterNode.length; i++) {
					counterNode[i].innerHTML = i + 1;
				}
			} else {
		...
		function clearRowTaxable(element) {
			...
			$tr.find('input[name="majorItemsTaxable[]"]')
					.prop('checked', false);
		...
		function clearRowNonTaxable(element) {
			...
			$tr.find('input').val('');
			$tr.find("td:eq(5)").html("");
			$tr.find('input[name="majorItemsNonTaxable[]"]').prop('checked',
					false);
			$tr.find('td:eq(3)').removeClass("errorVal");
			$tr.find('td:eq(4)').removeClass("errorVal");
		...
		function quotationDownload() {
			$
					.ajax({
						...
						url : serverHTTP + 'Quotation/QuotationCSVOutput',
						method : 'POST',
						data : {
							"quotationId" : searchid
						},
						...
						error : function(error) {
							console.log("Sorry error in file creation");
							console.log(JSON.stringify(error));
						}
					});
		}
